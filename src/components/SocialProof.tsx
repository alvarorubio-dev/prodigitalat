...
const testimonials = [
  {
    ...
    quote: 'La primera semana recibí 6 llamadas nuevas. Con los primeros dos trabajos ya recuperé lo que pagué por la web. Ahora me llaman directo de Google, sin depender de nadie.',
  ...
  },
  {
    ...
    quote: 'Todo el proceso fue por WhatsApp, en español. En 3 días tenía mi página lista. Ahora aparezco en Google cuando buscan "plomero Houston" y me llaman solos.',
  ...
  },
  {
    ...
    quote: 'Mis clientes piensan que soy una empresa grande. La página se ve muy profesional y me diferencia de la competencia. Cerré 4 contratos nuevos en el primer mes.',
...
export default function SocialProof() {
  ...
  return (
    <section className="py-24 bg-navy relative overflow-hidden" aria-labelledby="social-proof-heading">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.h3 variants={fadeInUp} className="text-brand-orange font-bold text-sm uppercase tracking-widest mb-4">
          ...
          <motion.div variants={fadeInUp} className="flex justify-center mb-6">
            <div className="inline-flex items-center gap-2.5 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-5 py-2.5">
              <GoogleLogo />
              <div className="flex items-center gap-1" role="img" aria-label="4.9 de 5 estrellas en Google">
          ...
          >
            ...
            <span className="text-gradient">intermediarios para conseguir clientes.</span>
          </motion.h2>
          <motion.p variants={fadeInUp} className="mt-4 text-slate-400 text-lg max-w-2xl mx-auto">
            Negocios reales, crecimiento constante. Estos contratistas latinos dejaron de pagar comisiones a terceros y ahora reciben contactos directos de Google.
          </motion.p>
        </motion.div>
        ...
        >
          {testimonials.map(({ name, trade, quote, result, resultDetail, rating, initials, color }) => (
            <motion.article
              key={name}
              variants={fadeInUp}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex gap-1" role="img" aria-label={`${rating} de 5 estrellas`}>
                  {Array.from({ length: rating }).map((_, i) => (
                    <Star key={i} className="w-4 h-4 text-yellow-400" fill="currentColor" aria-hidden="true" />
                  ))}
                </div>
                <GoogleLogo />
              </div>
              ...
              <div className="bg-brand-orange/10 border border-brand-orange/20 rounded-xl px-4 py-3 flex items-center gap-3">
                <TrendingUp className="w-4 h-4 text-brand-orange flex-shrink-0" aria-hidden="true" />
                <div>
                  <div className="text-white font-black text-sm">{result}</div>
                  <div className="text-slate-400 text-xs">{resultDetail}</div>
                </div>
              </div>

              <p className="text-slate-300 leading-relaxed text-sm flex-1">"{quote}"</p>
              <div className="flex items-center gap-3 pt-2 border-t border-white/10">
                <div className={`w-10 h-10 bg-gradient-to-br ${color} rounded-full flex items-center justify-center text-white font-black text-sm flex-shrink-0`} aria-hidden="true">
                  {initials}
                </div>
                <div>
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="text-orange-100 text-lg mb-7 max-w-xl mx-auto">
              Cada día que pasas sin una página web profesional, es una ventaja directa que le regalas a tu competencia.
            </p>
            <a
              href={WHATSAPP_URL}
              target="_blank"
