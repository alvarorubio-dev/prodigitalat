...
function BetsyRossFlag() {
  ...
  return (
    <svg viewBox="0 0 100 67" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
      ...
      {[0,1,2,3,4,5,6].map(i => (
        <rect key={i} x="0" y={i * 9.57 + (i > 0 ? 0.5 : 0)} width="100" height={i === 6 ? 9.07 : 9.07} fill={i % 2 === 0 ? '#B22234' : '#FFFFFF'} />
      ))}
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-18">
          <a href="#" className="flex items-center gap-2 group" aria-label="ProDigital.lat - Ir al inicio">
            <div className="w-8 h-8 rounded-lg overflow-hidden shadow-lg flex-shrink-0" aria-hidden="true">
              <BetsyRossFlag />
            </div>
          ...
          <nav className="hidden lg:flex items-center gap-8" aria-label="Navegación principal">
            {navLinks.map((link) => (
              ...
              >
                {link.label}
                <span className="absolute -bottom-0.5 left-0 w-0 h-0.5 bg-brand-orange transition-all duration-300 group-hover:w-full rounded-full" aria-hidden="true" />
              </a>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center">
            <a
              ...
              className="flex items-center gap-2 bg-brand-orange hover:bg-brand-orange-dark text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 shadow-lg shadow-orange-900/30 hover:shadow-orange-900/50 hover:-translate-y-0.5"
            >
          ...
          >
            <nav className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1" aria-label="Navegación móvil">
              ...
              <a
                ...
                onClick={() => setMenuOpen(false)}
                aria-label="Hablar por WhatsApp para cotización"
                className="mt-2 flex items-center justify-center gap-2 bg-brand-orange text-white font-semibold py-3 px-5 rounded-xl"
              >
                <MessageCircle className="w-4 h-4" aria-hidden="true" />
                Hablar por WhatsApp
              </a>
